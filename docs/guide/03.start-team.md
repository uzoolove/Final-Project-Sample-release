...
- [4.1 팀 레포지토리 fork](./03.start-team.md#4.1-팀-레포지토리-fork)에서 fork한 레포지토리가 속한 계정 선택(현재 로그인한 계정)
...
- Name: `TOKEN_[시크릿 토큰 생성한 계정]`
  - 예시: `TOKEN_YONG`
- Secret: 4.2 단계에서 복사한 시크릿 토큰 붙여넣기
- Add secret
...
```
  ...
  - `[시크릿 토큰을 저장한 변수]` 자리에 [7.3.3 토큰 변수 등록](./03-start-team.md#7.3.3-토큰-변수-등록)애서 등록한 Name 입력
    - 예시: secrets.TOKEN_YONG
  - `[fork한 레포지토리의 소유자]` 자리에 [7.2.4 Resource owner 선택](./03-start-team.md#7.2.4-Resource-owner-선택)에서 선택한 사용자
    - 예시: uzoolove
  - `[fork한 레포지토리명]` 자리에 [4.1.3 Repository name 입력](./03-start-team.md#4.1.3-Repository-name-입력)에서 입력한 레포지토리명
    - 예시: LionBoard
  - `[시크릿 토큰 소유자 이메일을 저장한 변수]` 자리에 [7.3.4 이메일 변수 등록](./03-start-team.md#7.3.4-이메일-변수-등록)에서 등록한 Name 입력
    - 예시: secrets.EMAIL_YONG
...
### 7.5.4 배포 결과 확인
- [4.2.3 테스트](./02.start.md#4.2.3-테스트)에서 배포된 주소로 접속 후 수정 내역이 반영되었는지 확인
